...
public class StorageNode implements DFSNode {
  ...
  /* Message router to listen for incoming messages */
  ...
  /* Controller hostname to connect to */
  String controllerHostName;

  /* A directory in which the dfs can store data  (input by user -r directory)*/
  Path rootDirectory;

  /* Command line arguments */
  ArgumentMap arguments;

  /* Our current replica assignments */
  ArrayList<String> replicaHosts;

  /* Bootstrap to use */
  Bootstrap bootstrap;

  /* Save channel context of client when requesting healed chunks from other nodes */
  ChannelHandlerContext clientCtx;

  /* Filenames -> Chunks. This will allow us to quickly access chunk paths on retrieval */
  HashMap<String, ArrayList<ChunkWrapper>> chunkMap;

  /* Hostname -> Chunks. This will help us to determine and change ownership of data */
  HashMap<String, ArrayList<ChunkWrapper>> hostnameToChunks;
  ...
  public StorageNode(String[] args) throws UnknownHostException {
    ...
    /* Initialize bootstrap with i/o workerGroup and pipeline */
    EventLoopGroup workerGroup = new NioEventLoopGroup();
    /* Pass reference to this nodes interface */
  ...
  public static void main(String[] args) throws IOException {
    ...
    /** Connect to controller and send join request */
    ...
    StorageMessages.StorageMessageWrapper msgWrapper =
        Builders.buildJoinRequest(storageNode.hostname);
    ...
    HeartBeatRunner heartBeat =
        new HeartBeatRunner(
            storageNode.hostname, storageNode.controllerHostName, storageNode.bootstrap);
    Thread heartThread = new Thread(heartBeat);
    heartThread.start();
    logger.info("Started heartbeat thread.");

    /* Start the message router to listen for inbound requests */
    storageNode.start();
  }
  ...
  public void onMessage(ChannelHandlerContext ctx, StorageMessageWrapper message) {
    if (message.hasStoreRequest()) {
      /** If we get a store request for a file we will delete the file if we have it */
      StorageMessages.StoreRequest storeRequest = message.getStoreRequest();

      logger.info(
          "Request to store "
              + storeRequest.getFileName()
              + " size: "
      ...
      logger.info(
          "Replica Assignments "
              + message.getReplicaAssignments().getReplica1()
              + " "
              + message.getReplicaAssignments().getReplica2());
      ...
      /* Store metadata */
      hostnameToChunks.putIfAbsent(
          message.getStoreChunk().getOriginHost(), new ArrayList<ChunkWrapper>());
      ...
      if (replicaHosts.size() == 2
          && message.getStoreChunk().getOriginHost().equals(this.hostname)) {
      ...
      logger.info(
          "Attempting to shoot chunks of "
              + message.getRetrieveFile().getFileName()
              + " to client");
      ...
      /**
       * If we get a heal request we need to write back a healed chunk if we have it, otherwise we
       * need to send a request to the final location of the chunk
       */
      ...
      if (healRequest.getIntermediateLocation().equals(this.hostname)) {
        ...
        if ((healResponse = this.getChunkAsHealResponse(message.getHealRequest())) != null) {
          ...
          ChannelFuture cf =
              bootstrap.connect(healRequest.getInitialLocation(), 13114).syncUninterruptibly();
          ...
          ChannelFuture requestAgain =
              this.bootstrap.connect(healRequest.getFinalLocation(), 13114).syncUninterruptibly();
        ...
        /**
         * If our chunk matches its checksum we can send it back to intermediate location, Otherwise
         * all of our data is corrupted and we cannot retrieve it
         */
        if ((healResponse = this.getChunkAsHealResponse(message.getHealRequest())) != null) {
          ChannelFuture cf =
              bootstrap.connect(healRequest.getIntermediateLocation(), 13114).syncUninterruptibly();
          cf.channel().writeAndFlush(healResponse).syncUninterruptibly();
        } else {
          logger.info(
              "All data corrupted for "
                  + healRequest.getFileName()
                  + " chunk "
      ...
      StorageMessages.StorageMessageWrapper chunk =
          Builders.buildStoreChunk(
              healResponse.getFileName(),
              healResponse.getPassTo(),
              healResponse.getChunkId(),
              healResponse.getTotalChunks(),
      ...
      StorageMessages.ReplicateToNewAssignment replicaRequest =
          message.getReplicateToNewAssignment();
      String downNode = replicaRequest.getDownNode();
      String newReplicaAssignment = replicaRequest.getNewAssignment();

      logger.info(
          "Recieved Replication to new assignment: Down Node: "
              + downNode
      ...
      synchronized (replicaHosts) {
        if (this.replicaHosts.get(0).equals(downNode)) {
          logger.info(
              "Took new assignment "
                  + newReplicaAssignment
                  + " because old assignment "
                  + this.replicaHosts.get(0)
                  + " went down.");
      ...
      synchronized (hostnameToChunks) {
        ArrayList<ChunkWrapper> pathsToThisNodesPrimaries =
            this.hostnameToChunks.get(this.hostname);

        logger.info(
            "This Storage Nodes chunks that we will be re-replicating to "
                + newReplicaAssignment
                + " --->");
      ...
      synchronized (hostnameToChunks) {
        ...
        synchronized (replicaHosts) {
          for (String host : replicaHosts) {
            ...
            /**
             * If we have one of down node's assignment's then we don't want to delete data, we will
             * simply merge location2's replicas of downNode and location2's replica's of this under
             * the key this.hostname
             */
            if (host.equals(location2)) {
              delete = false;
              /* This protobuf means merge downNode's chunks into this.hostnames and delete the key downNode */
              ChannelFuture write =
                  chan.writeAndFlush(Builders.buildSimplyMerge(this.hostname, downNode));
              write.syncUninterruptibly();
            } else {
      ...
      /**
       * If we get this message, another node handled rereplication of this data and we can delete
       * the key.
       */
   ...
   * @return healedchunk
   */
  public StorageMessages.StorageMessageWrapper getChunkAsHealResponse(
      StorageMessages.HealRequest healRequest) {
    ...
    logger.info(
        "Request to heal " + healRequest.getFileName() + " chunk " + healRequest.getChunkId());
        ...
        if (chunk.chunkID == chunkID) {
          try {
            ...
            if (chunk.checksum.equals(Checksum.SHAsum(data))) {
              ...
              StorageMessages.HealResponse validChunk =
                  StorageMessages.HealResponse.newBuilder()
  ...
  /**
   * Find all chunks // tokenize them check metadata // heal if neccessary // send chunks to client
   *
   * @param filePath {@link ChunkFinder} Produce a list of all chunks from root file directory
   */
          ...
          for (ChunkWrapper chunk : chunks) {
            ...
            if (Files.exists(chunkPath)) {
              ...
              if (!checksum.equals(checksumCheck)) {
                /**
                 * If our the checksum of a chunk no longer matches we will send a request To our
                 * first replica assignment to send back a valid chunk
                 */
                ...
                StorageMessages.StorageMessageWrapper chunkToSend =
                    Builders.buildStoreChunk(
                        chunk.getFileName(),
                        this.hostname,
                        chunk.getChunkID(),
  ...
  /**
   * Writes a StoreChunk to disk and appends its checksum to the filename The chunk will be stored
   * under the root/home directory entered in the command line on storage node startup This is done
   * with the -r flag
   *
   * <p>Home - file_chunks - chunk0#AD12341FFC... chunk1#AD12341111... chunk2#12341FFC11...
   *
   * <p>file_chunks will be named the name of the file whose chunks we are storing we will also
   * append a chunks chunkid, #, and the sha1sum of the given file's bytes.
   *
   * <p>This will allow us to verify to correctness of the data on retrieval
   *
   * @param message
   */

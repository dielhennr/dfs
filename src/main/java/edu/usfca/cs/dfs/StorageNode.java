...
public class StorageNode implements DFSNode {
	...
	public void onMessage(ChannelHandlerContext ctx, StorageMessageWrapper message) {
		if (message.hasStoreRequest()) {
				...
				for (ChunkWrapper chunk : pathsToThisNodesPrimaries) {
					try {
						...
						writes.add(cf.channel().writeAndFlush(Builders.buildStoreChunk(chunk.getFileName(),
								this.hostname, chunk.getChunkID(), chunk.getTotalChunks(), ByteString.copyFrom(data))));
					} catch (IOException ioe) {
						logger.info("Could not send " + chunk.getFileName() + " id " + chunk.getChunkID() + " to "
								+ newReplicaAssignment);
					}
				}
			...
			/*
			 * This means that we are dealing with a storage node that was receiving
			 * replicas from the down node. In this case, this node needs to be designated
			 * the new primary for the down nodes data, and it needs to replicate it's down
			 * nodes data to its replica assignments.
			 *
			 * down node / \ / \ this location2
			 *
			 * We will look at this nodes replica assignments. If we are assigned to the
			 * same node that the down node was assigned to, we just need to send that node
			 * a message telling it to merge the down node's chunks with ours in its map and
			 * remove the down node's key. Then this will send down node's chunks to it's
			 * other replica assignment Case A this / \ / \ location2 this.otherAssignment
			 * 
			 * Where this.otherAssignment is the one we will send all of down node's chunks
			 * to location2 already has down node's chunks, so we need to merge the chunks
			 * of downNode and this under this.hostname in location2
			 *
			 * The other case is when this has totally different replica locations than the
			 * down node If this is the case we can send a message to the other location of
			 * the down nodes data to have it delete it. Then we can replicate down nodes
			 * data from this to this.Assigment1 and this.assignment2 under this.hostname
			 *
			 * Case B this ----- > "hey location2 verify/delete down nodes data" / \ / \
			...
			synchronized (hostnameToChunks) {
				...
				/*
				 * If we aren't assigned to downNodes other assignment we need to delete the
				 * data on that node or use it to verify the correctess of our data
				 */
				boolean delete = true;
				for (String host : replicaHosts) {
					...
					/**
					 * If we have one of down node's assignment's then we don't want to delete data,
					 * we will simply merge location2's replicas of downNode and location2's
					 * replica's of this under the key this.hostname
					 */
					if (host.equals(location2)) {
						delete = false;
						/*
						 * This protobuf means merge downNode's chunks into this.hostnames and delete
						 * the key downNode
						 */
						...
						/*
						 * Otherwise we need to rereplicate the data to our assignments and tell them
						 * that we are the primary holder as always
						 */
						ArrayList<ChannelFuture> writes = new ArrayList<>();
						...
						for (ChunkWrapper chunk : pathsToDownNodesData) {
							try {
								...
								StorageMessages.StorageMessageWrapper replicaChunk = Builders.buildStoreChunk(
				...
				/*
				 * If none of our assignments were the down nodes other assignment, send a
				 * request to the down node's other assignment to delete its key for the down
				 * node
				 */
				if (delete) {
			...
			/**
			 * If we get this message, another node handled rereplication of this data and
	...
	public void writeChunk(StorageMessages.StorageMessageWrapper message) {
		...
		try {
			 ...
			 */
			ChunkWrapper chunk = new ChunkWrapper(path, fileName, message.getStoreChunk().getChunkId(),
